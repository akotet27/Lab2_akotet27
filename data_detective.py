...
# QUEST 1 — removes tweets with no text and replaces blank likes/retweets with 0
# CSV files store everything as text so I also convert likes and retweets to integers
...
# QUEST 3 — sorts all tweets from most liked to least liked using Selection Sort
# I am NOT allowed to use .sort() or sorted() so I write the algorithm myself
# How it works: each pass finds the highest liked tweet and swaps it to the front
# Example: [30,10,50,20,40] → Pass1 finds 50 → [50,10,30,20,40] → Pass2 finds 40 → [50,40,30,20,10]
...
# calls sort_posts_by_likes and prints the top 10 results
...
# QUEST 4 — asks the user for a keyword and finds all tweets that contain it
# the search is case-insensitive so 'music' also finds 'Music', 'MUSIC' etc.
# after each search I ask if they want to search again — they can keep going
# until they type 'no' to stop
def search_posts(good_posts):
    # I use this flag to control the search loop
    # as long as keep_searching is True, I will ask for another keyword
    keep_searching = True
    while keep_searching:
        ...
        while True:
            ...
            if again in ('yes', 'y'):
                # user wants to search again so I break out and loop back
                break
            elif again in ('no', 'n'):
                # user is done so I stop the search loop
                keep_searching = False
                print("  Okay, I am done searching.")
                break
            else:
                # user typed something invalid like a number or random text
                print(f"  '{again}' is not a valid answer. Please type yes or no.")


# MAIN — calls each quest function in order and passes results along
def main():
