...
| `feed-analyzer.sh` | Bash script — finds the top 5 most active users |
...
Make sure `twitter_dataset.csv` is in the same folder as `data-detective.py`, then run:

```bash
python data-detective.py
```

The script will automatically run all 4 quests one by one.
At Quest 4 it will ask you to type a search keyword — just type any word and press Enter.

Example:
```
Enter a search keyword: love
Found 12 tweet(s) matching 'love'
```
...
Uses a manual loop instead of Python's built-in `max()` function.
...
Asks the user to enter a search keyword.
Searches every tweet for that keyword (case-insensitive).
Collects all matches into a new list and prints how many were found, followed by the tweets themselves.
...
The idea is simple: go through the list position by position. At each position, scan everything after it to find the tweet with the most likes. Once found, swap it into the current position. Repeat until the whole list is sorted.
...
# Output: Usage: feed-analyzer.sh twitter_dataset.csv
```

**If the file does not exist:**
```bash
bash feed-analyzer.sh wrong_name.csv
# Output: File not found: wrong_name.csv
```

**If the file has no data rows:**
```bash
bash feed-analyzer.sh empty.csv
